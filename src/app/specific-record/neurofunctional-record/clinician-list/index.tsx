...
export default function NeuroList() {
  const { data, loading, error, handleFetchByClinicianId } = useNeuro();
  ...
  const handleCreateRecord = () => {
    router.push(`/specific-record/search-patient/neuro`);
  };
  ...
  return (
    <SafeAreaProvider style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        <Text style={styles.title}>Fichas Neurofuncionais</Text>
        <TouchableOpacity
        ...
        {!loading && records.length > 0 && (
          <ScrollView style={styles.scrollView}>
            {records.map((record: any) => (
              <View key={record.neurofunctionalRecordId} style={styles.card}>
