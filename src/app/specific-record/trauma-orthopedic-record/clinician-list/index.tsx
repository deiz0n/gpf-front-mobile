...
export default function TraumaList() {
  const { data, loading, error, handleFetchByClinicianId } =
    useTraumaOrthopedic();
  ...
  const handleCreateRecord = () => {
    router.push(`/specific-record/search-patient/trauma`);
  };
  ...
  return (
    <SafeAreaProvider style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        <Text style={styles.title}>Fichas Traumato Ortopédicas</Text>
        ...
        {!loading && records.length > 0 && (
          <ScrollView style={styles.scrollView}>
            {records.map((record: any) => (
              <View key={record.traumaOrthopedicRecordId} style={styles.card}>
      ...
      {/* Paginação fixa no final da tela */}
